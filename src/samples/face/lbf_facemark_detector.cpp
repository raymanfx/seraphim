...
int main(int argc, char **argv) {
    ...
    int opt = 0;
    int long_index = 0;
    while ((opt = getopt_long(argc, argv, "i:m:c:h", long_opts, &long_index)) != -1) {
        switch (opt) {
        case 'i':
    ...
    if (!viewer.create("LBF Facemark Detector")) {
        return 1;
    }
